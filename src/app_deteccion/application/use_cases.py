...
class RegisterCriticalProductUseCase:
    ...
    def execute(self, command: RegisterCaseCommand) -> CriticalProductCase:
        ...
        case = CriticalProductCase(
            ...
            evidence_photo_name=command.evidence_photo_name,
            evidence_photo_data=command.evidence_photo_data,
...
class ValidateCaseBySupervisorUseCase:
    ...
    def execute(self, case_id: str, supervisor_user: str, decision: str = "APROBADO", comment: str = "") -> CriticalProductCase:
        case = self.repository.get(case_id)
        if case is None:
            raise DomainValidationError("No existe el caso solicitado")
        case.validate_by_supervisor(supervisor_user, decision=decision, comment=comment)
...
class TraceabilityQuery:
    def execute(self) -> dict:
        return {
            ...
            "design_doc_features": "docs/design/DD-FEAT-001-002-demo-aplicada.md",
            "adr": "docs/adr/ADR-0006-demo-monolito-modular-fastapi-sqlite.md",
            "adr_features": "docs/adr/ADR-0007-ui-demo-aplicada-fastapi-html.md",
            "prompt": "docs/prompts/impl/PR-IMPL-001-registro-producto-critico.md",
            "prompt_features": "docs/prompts/impl/PR-IMPL-002-dos-features-demo-aplicada.md",
            "code": [
                "src/app_deteccion/domain",
                "src/app_deteccion/application",
                "src/app_deteccion/adapters",
                "src/app_deteccion/adapters/web_ui.py",
                "src/app_deteccion/infrastructure",
            ],
            "tests": ["tests/test_domain.py", "tests/test_application.py", "tests/test_api.py", "tests/test_ui_routes.py"],
